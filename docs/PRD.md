...
- **Merge-tail recovery (ADR 0006, ADR 0009):** if the merge gate `blocked`s on a **babysit-able** reason (required CI red / not-mergeable conflicts / unresolved review comments), run the conditional `/babysit` agent phase (cold `cursor("auto")`, can push commits, `maxIterations > 1`) then re-run the merge gate **once**. Cap = 1 attempt; still blocked → mark `blocked` for the operator. **Human** reasons (no clean `Approve` on the handoff used at merge gate, review-pr still on `/review-tdd` route, logical blockers) skip babysit and block immediately. Red CI **after** `/review-tdd` still reaches merge and may trigger babysit — not a hard stop at review-tdd. `/babysit` is not in the linear loop and never runs on the happy path.
- **review-pr `blockers` (ADR 0009):** on **request-changes** with `nextSkill: "/review-tdd"`, populated `blockers` are review findings for the next phase, not a pipeline halt.
